...
class FiscalYearShifter:
  ...
  def AssociateFiscalEndWithTrends(self, date, trend, term, worldFlag):
    flag = 0
    FiscalEnd = self.TermDict[term][0].strip()
    if FiscalEnd == "Mar" or FiscalEnd == "Jun" or FiscalEnd == "Sep" or FiscalEnd == "Dec": # fiscal quarter end = calendar quarter end
      pass # do nothing
    elif FiscalEnd == "Feb" or FiscalEnd == "May" or FiscalEnd == "Aug" or FiscalEnd == "Nov":
      print FiscalEnd
      flag = 4 # trends data shift down 4 cells/rows
    elif FiscalEnd == "Jan" or FiscalEnd == "Apr" or FiscalEnd == "Jul" or FiscalEnd == "Oct":
      print FiscalEnd
